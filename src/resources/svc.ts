export class SVC {
	...
	constructor(
		private readonly kernel = (input: number[]): number[] => input,
	...
	public train(inputs: number[][], expect: number[]): void {
		this.inputs = inputs.map(this.kernel);
		this.expect = expect;

		// init internalss
		this.a = new Array(this.inputs[0].length).fill(0);
		this.b = 0;

		// step forward
		for (let epochs = 0; epochs < this.numEpochs; epochs++) {
			for (let i = 0; i < this.inputs.length; i++) {
				if (expect[i] * this.margin(this.inputs[i]) < 1) {
					this.adjustNegative(this.inputs[i], expect[i]);
				} else {
					this.adjustPositive();
				}
			}
